...
function ReportResult({report}:{report:ReportPage}){return <><div className="metric-definitions">{report.metric_definitions.map(m=><article key={m.metric_key}><strong>{m.metric_name}</strong><p>{m.definition}</p>{m.denominator_definition&&<small>Denominator: {m.denominator_definition}</small>}</article>)}</div><div className="table-wrap" tabIndex={0}><table><thead><tr>{report.columns.map(c=><th key={c}>{c.replaceAll('_',' ')}</th>)}</tr></thead><tbody>{report.items.map((row,index)=><tr key={index}>{report.columns.map(c=><td key={c}>{display(row[c])}</td>)}</tr>)}</tbody></table></div>{!report.items.length&&<p className="empty-state">This report has no rows.</p>}</>}
