...
English Class Management — canonical v3. Local single-machine test deployment:
React + FastAPI on Python 3.13 over PostgreSQL 17. Named users sign in through
server-side sessions; every write is one service transaction with an audit
event.

## Layering (strict, enforced by the phase 7 gate)

```
web/ React components       browser UI; calls only /api/*
  └─ api/ FastAPI           auth, validation, HTTP contracts
       └─ services/         one command = one transaction + audit (RBAC here)
       └─ frontend_queries.py  read models (SELECT-only, parameterized)
            └─ PostgreSQL   migrations/ 001..020: schema, triggers, views
```

- UI never embeds SQL; FastAPI reads through registered query modules and
  writes through `services.BusinessService`.
...
| Path | Purpose |
|---|---|
| `api/` | FastAPI routes, same-origin auth/session boundary, validation and observability. |
| `web/` | React UI, unit tests, Playwright journeys and production build. |
...
| `frontend_queries.py` | Task-oriented read models shared by API routes. |
...
| `backup.ps1`, `run_react_app.cmd/.ps1` | Verified daily backup (scheduled 12:00) and checked FastAPI launcher. |
