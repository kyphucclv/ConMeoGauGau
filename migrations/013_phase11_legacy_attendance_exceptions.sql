-- Owner-approved exception for historical delivered sessions whose attendance
-- source is unavailable.  This never creates a Present/Absent attendance fact.
-- Grain: one exception covers all missing applicable roster entries in one session unit.
-- Forward verification: approved rows remove only their matching roster issue.
-- Rollback: restore the pre-exception backup; do not delete approval audit selectively.

CREATE TABLE attendance_roster_legacy_exceptions (
    session_unit_id BIGINT PRIMARY KEY REFERENCES session_units(session_unit_id),
    reason TEXT NOT NULL CHECK (BTRIM(reason) <> ''),
    approved_by_user_id BIGINT NOT NULL REFERENCES app_users(user_id),
    approved_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

COMMENT ON TABLE attendance_roster_legacy_exceptions IS
    'Audited approval that a historical session roster is unavailable; no attendance status is inferred.';
